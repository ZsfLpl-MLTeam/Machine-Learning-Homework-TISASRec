...
This variant removes absolute position attention and only uses relative time-intervals.
"""
...
class TiSASRec_R(SequentialModel):
    reader = 'SeqReader'
    runner = 'BaseRunner'
    ...
    def _define_params(self):
        ...
        # Remove absolute position embeddings (p_k_embeddings, p_v_embeddings)
    ...
    def forward(self, feed_dict):
        self.check_list = []
        i_ids = feed_dict['item_id']  # [batch_size, -1] 候选物品ID
        i_history = feed_dict['history_items']  # [batch_size, history_max] 历史物品序列
        t_history = feed_dict['history_times']  # [batch_size, history_max] 历史时间戳
        user_min_t = feed_dict['user_min_intervals']  # [batch_size] 用户最小时间间隔
        ...
        # We remove absolute position embeddings: use zero tensors instead
        pos_k = torch.zeros_like(his_vectors)
        pos_v = torch.zeros_like(his_vectors)
...
class TimeIntervalMultiHeadAttention(nn.Module):
    def __init__(self, d_model, n_heads, kq_same=False, bias=True):
        super().__init__()
        """
        It also needs interaction (time interval) key/value input.
        Absolute position is not used in this variant.
        """
    ...
    def forward(self, q, k, v, pos_k, pos_v, inter_k, inter_v, mask):
        ...
        # pos_k and pos_v are expected but will be zero tensors in this variant
