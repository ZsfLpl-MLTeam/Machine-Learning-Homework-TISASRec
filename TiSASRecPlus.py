...
class ContinuousTimeEncoding(nn.Module):
    def __init__(self, d_model):
        super(ContinuousTimeEncoding, self).__init__()
        self.d_model = d_model
        freq_list = [10 ** (i / (d_model // 2) * 4 - 3) for i in range(d_model // 2)]
        self.w = nn.Parameter(torch.tensor(freq_list).view(1, -1))
        self.b = nn.Parameter(torch.zeros(1, d_model // 2))

    def forward(self, time_diff):
        # time_diff: [batch_size, seq_len, seq_len]
        log_time = torch.log(time_diff + 1.0) 
        x = log_time.unsqueeze(-1) * self.w 
        x = x + self.b
...
class TiSASRecPlus(SequentialModel):
    ...
    def _define_params(self):
        ...
        self.t_k_embeddings = ContinuousTimeEncoding(self.emb_size)
        self.t_v_embeddings = ContinuousTimeEncoding(self.emb_size)
    ...
    def forward(self, feed_dict):
        ...
        interval_float = interval_matrix.float() / (user_min_t.view(-1, 1, 1).float() + 1e-9)
        inter_k = self.t_k_embeddings(interval_float)
        inter_v = self.t_v_embeddings(interval_float)
